...
menuentry \"Arch Linux\" {
    ...
    options             \"rd.luks.name=${CRYPT_UUID}=crypt root=/dev/mapper/crypt rootflags=subvol=@ rw resume=UUID=${CRYPT_UUID} resume_offset=${RESUME_OFFSET}\"

    # options             \"rd.luks.name=${CRYPT_UUID}=crypt root=/dev/mapper/crypt rootflags=subvol=@ rw resume=UUID=${CRYPT_UUID} resume_offset=${RESUME_OFFSET} nvidia-drm.modeset=1 nvidia-drm.fbdev=1 nvidia.NVreg_RegistryDwords=EnableBrightnessControl=1 nvidia.NVreg_UsePageAttributeTable=1 nvidia.NVreg_PreserveVideoMemoryAllocations=1'\"

    # options             \"root=UUID=<UUID> resume=UUID=${CRYPT_UUID} resume_offset=${RESUME_OFFSET}\"
